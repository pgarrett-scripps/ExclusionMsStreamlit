...
if st.button('query'):
    ...
    try:
        excluded_intervals = exclusionms.apihandler.get_intervals_from_point(exclusion_api_ip=EXCLUSION_MS_API_IP,exclusion_point=exclusion_point)
        st.subheader('Overlapping ExclusionIntervals:')
        st.write(excluded_intervals)
